...
let reConnect = () => {
    ...
    rec = setTimeout(function () { // 延迟2秒重连  避免过多次过频繁请求重连
        createWebSocket();
    }, 2000);
};
//设置关闭连接
...
function initWebSocket() {
    // ws地址 -->这里是你的请求路径
    var ws = "ws://119.23.62.145:9100/ws"
    websock = new WebSocket(ws,[getToken()])
...
function sendSock(agentData, callback) {
    // console.log(agentData,websock.readyState,websock.OPEN,websock.CONNECTING)
...
function websocketonmessage(e) {
    ...
    if (!O_o) {
        ...
        if (O_o.msg == "open success") {
            ...
            // console.log(O_o,'-------------------');
            if(O_o.method=="new_message"){
                // this.$store.commit('SET_WEBSOCKET',O_o)
                $bus.emit('new_message',O_o);
            }
            globalCallback(O_o);
        }
    }
