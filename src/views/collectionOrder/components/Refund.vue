<template>
    <div>
        <DirectiveDialog ref="dialog" :title="`订单号${orderDetail.order_no}`"  :refName="refName">
            <div class="content">
                <div class="message" >订单收款成功，收款金额：<span class="num">{{ orderDetail.order_money }} </span>将在N天后到账户余额</div>
                ...
                <div class="form">
                    ...
                    <div class="flex-row">
                        <div class="form-items" style="width:60%;">
                            ...
                            <el-upload class="upload-com" action="" :fileList="[]" :show-file-list="false"
                                ...
                                :limit="1">
                                <!-- <div class="form-item-title form-item-see">上传附件</div> -->
                                <div class="form-item-title form-item-see" v-if="!refund_images">上传附件</div>
                                <div class="form-item-title form-item-see" v-if="refund_images">附件已上传</div>
                            </el-upload>
                        </div>
                        <!-- <el-badge :value="3"> -->
                        <!-- <div class="form-items">
                ...
                <div class="submit-btn">
                    <div class="btn1 flex-content"  v-if="[8].includes(orderDetail.order_status)&&userInfo.identity==1" @click="handleRefund">退款</div>
                    <div class="btn3 flex-content" v-if="[8,9].includes(orderDetail.order_status)&&userInfo.identity==2" @click="handleSend">收款争议</div>
...
import {  order_detail ,uploadImg,order_refund,order_received_argue} from '@/api/project'
...
export default {
    ...
    methods: {
        ...
        async handleRefund(){
            if(!this.refund_images){
                this.$message.error('请先上传图片证明')
                return
            }
            let res = await order_refund({ order_id: this.orderDetail.order_id,refund_images:this.refund_images });
            if (res.code == 200) {
                this.$message.success(res.msg)
                this.getDetail(this.orderDetail.order_id)
                return
