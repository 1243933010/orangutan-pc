          ...
          <el-form ref="basicData" class="basic-data-form" :model="basicData" label-width="94px">
            ...
            <div class="right-form">
              <el-form-item label="用户头像" class="input-box">
                ...
                <el-upload class="upload-com" action="" :file-list="[]" :http-request="(files) => httpRequest1(files)"
              ...
              <el-form-item label="账号名称" class="input-box label-border">
                ...
                <el-button v-if="userBool.userName" class="no-bg-btn" @click="setUser('username')">修改</el-button>
              </el-form-item>
        ...
        <div class="tab-pane-container" style="border-radius: 10px; text-align: center">
          <el-table height="100%" :data="receivingAddressList" style="width: 100%">
            ...
            <el-table-column label="操作">
              ...
              <template slot-scope="scope">
                <el-button type="primary" plain @click="handleDel(scope.row)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
...
import AddressForm from '@/views/personalData/components/addressForm'
...
import {
  ...
  uploadImg, user_info_edit, member_wallet_list, member_wallet_add, member_wallet_edit, member_wallet_del
} from '@/api/project'
...
export default {
  ...
  methods: {
    async handleDel(row) {
      const res = await member_wallet_del({ member_wallet_id: row.member_wallet_id })
      if (res.code == 200) {
        this.$message.success(res.msg)
        this.getMemberWalletList();
        return
      }
      this.$message.error(res.msg)
    },
    async getMemberWalletList() {
      let res = await member_wallet_list();
      if (res.code == 200) {
        this.receivingAddressList = res.data;
      }
    },
    async setUser(type) {
    ...
    async getDialogData(data) {
      // console.log(data);
      let fnc = member_wallet_add
      if (data.member_wallet_id) {
        fnc = member_wallet_edit
      }
